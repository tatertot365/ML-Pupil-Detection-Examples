	...
	if circles is not None:
		...
		for j,k in blackCoordinates:
			if a == int(len(blackCoordinates)/2):
				print('Pupil Point Black Pixels')
				pupilPoint = [k,j]
			a += 1
...
for imageName in os.listdir(args["image"]):
	...
	# write coordinates of pupils on the image in the bottom left corner
	cv2.putText(image, str(leftPupilPoint), (10, image.shape[0] - 100), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (255, 0, 0), 1)
	cv2.putText(image, str(rightPupilPoint), (10, image.shape[0] - 20), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (255, 0, 0), 1)
